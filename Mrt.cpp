...
int Title(){	//返り値：title_tag  引数：なし	
	...
	while( BasicLoop() && !CheckHitKeyDown(KEY_INPUT_ESCAPE)){
		...
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*1/10), WHITE, "- 天 佑 -");
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*5/10), WHITE, "つづきから");
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*6/10), WHITE, "はじめから");
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*7/10), WHITE, "　設定　　");
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*8/10), WHITE, "ゲーム終了");
...
int ChooseData(playdata_tag* _playdata){	//返り値：ロードするデータ番号（エラー：-1[backtotitle]）  引数：セーブデータ配列
	...
	while( BasicLoop()){
		...
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*1/4), WHITE, "ロードするデータを選択");
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*2/4), (_playdata[dnum].Exist? WHITE:GRAY), tmpstring);
		DrawCenterString(WINDOW_WIDTH/2, (int)(WINDOW_HEIGHT*3/4), WHITE, "↑↓:データ選択　Z:決定　Esc:タイトルに戻る");

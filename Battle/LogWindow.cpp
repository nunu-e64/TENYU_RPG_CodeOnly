...
bool CLogWindow::Add(char *_format, ...){		//コメント行や空白行はLoadの段階で排除されている
	va_list args;
	...
	if (GetDrawStringWidthToHandle(newText, strlen(newText), FontHandle) > WordWidth){			//字数オーバー時処理 WordWidthはInit()で定義
	...
	}else if (mystrlen(newText) > WordNum){			//字数オーバー時処理（実際は描画サイズで処理するように上記に書き足したためこちらを使うことはないと思われる）
		char chOverstring[WORD_MAX];
		char chTruestring[WORD_MAX];
		
		int d = (_ismbblead(newText[WordNum-1])? --WordNum:0);	//行末が全角文字の1バイト目だった場合、文字化けするので1バイトずらす
